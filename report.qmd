...
res_06 = list()
res_06$pred = pred_06[[14]]
res_06$fit = resultados_06[[14]]
...
dat$Ct = as.vector(df_effort$catch_otb[as.numeric(as.character(df_effort$year_sale))>2005]/
                     df_effort$res[as.numeric(as.character(df_effort$year_sale))>2005])  # catches
dat$Et = as.vector(df_effort$effort_otb[as.numeric(as.character(df_effort$year_sale))>2005]) # effort
...
par$logalpha = log(1)
par$logbeta = log(1)
par$logK = log(0.000001)
par$logN0_scaled = log(0.1) # inserir nmult
par$logM = log(0.00001)
...
jnll = function(par){
  ...
  for(mes in 2:length(dat$Ct)){
    ...
    part2 = 0
      # if(mes != 1){
    for(mes_ant in seq_len(length(catch))){
