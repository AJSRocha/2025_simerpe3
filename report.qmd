...
xtable(data.frame(
                  ...
                  M = c(cat_pars_1$`M.MIS+OTB-S`,  exp(spg1['logM'])%>% unlist),
                  N0 = c(cat_pars_1$`N0.MIS+OTB-S`,  exp(spg1['logN0_scaled'])%>% unlist),
                  SD_Ct = c(cat_pars_1$`psi.MIS+OTB-S`, exp(spg1["logsdCt"])%>% unlist),
                  ...
                  row.names = c('CatDyn', 'RTMB_spg')) %>% t, digits = 8) %>%
  print(comment = F,
        sanitize.text.function = function(x)
          {gsub("_", "\\\\_", x)})
