...
Throughout this document, two example datasets will be used. Both datasets were presented by the $CatDyn$ package author during a workshop on this methodology. 
...
The objective function constitutes the calculation of the mean of the statistical process that is being modeled, and is the basis against which the parameters are optimized. According to several papers publised by the author, the process models catch at time $t$, $C_t$ and can be described as:
...
In $R$, the objective function is stated both in the *CatDynPred()* and in the several .CDMN auxiliary functions. No differences were detected in the implementations on each case, and it can be abbreviated as such: 
